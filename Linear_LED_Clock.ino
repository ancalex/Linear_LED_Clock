 ...
 * based on ESP 8266 Arduino IDE WebConfig by John Lassen https://web.archive.org/web/20170506190708/http://www.john-lassen.de/en/projects/esp-8266-arduino-ide-webconfig
 * and
...
void setup() {
	...
	if ((WIFI_connected != WL_CONNECTED) or !CFG_saved) {
		...
		config.ssid = "LinearClock-" + String(ESP.getChipId(), HEX); // SSID of access point
		config.password = "";
		config.dhcp = true;
		config.IP[0] = 192;
		config.IP[1] = 168;
		...
		config.DeviceName = "Linear Desktop Clock";
		...
		config.SecondColor = "#00FFFF";
		config.UnitMinuteColor = "#0000FF";
		config.TenthMinuteColor = "#00FF00";
		config.UnitHourColor = "#FF0000";
		config.TenthHourColor = "#FFFF00";
		config.BlinkColor = "#FF00FF";
		WriteConfig();
		WiFi.mode(WIFI_AP);
...
void loop() {
	...
	if (WIFI_connected != WL_CONNECTED and manual_time_set == false) {
		...
		FastLED.show();
			...
			if (temp_second != DateTime.second or temp_second == 0) {
				temp_second = DateTime.second;
				ColorInit();
				TimeDisplay();
				FastLED.show();
			}
		if (config.BlinkingEffect == 1) {
			pulse_led();
		}
	}
}

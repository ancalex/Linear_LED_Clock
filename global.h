...
CRGB solid_color;
CRGB second_color;
CRGB unit_minute_color;
CRGB tenth_minute_color;
CRGB unit_hour_color;
CRGB tenth_hour_color;
CRGB blink_color;
...
struct strConfig {
  ...
  String SecondColor;
  String UnitMinuteColor;
  String TenthMinuteColor;
  String UnitHourColor;
  String TenthHourColor;
  String BlinkColor;
  boolean ShowSeconds;
  boolean BlinkingEffect;
} config;
...
void WriteConfig(){
  ...
  // Application Settings here... from EEPROM 192 up to 511 (0 - 511)
  WriteStringToEEPROM(192, config.SecondColor);
  WriteStringToEEPROM(202, config.UnitMinuteColor);
  WriteStringToEEPROM(212, config.TenthMinuteColor);
  WriteStringToEEPROM(222, config.UnitHourColor);
  WriteStringToEEPROM(232, config.TenthHourColor);
  WriteStringToEEPROM(242, config.BlinkColor);
  EEPROM.write(252, config.ShowSeconds);
  EEPROM.write(254, config.BlinkingEffect);
  EEPROM.commit();
}
...
boolean ReadConfig(){
  ...
  if (EEPROM.read(0) == 'C' && EEPROM.read(1) == 'F'  && EEPROM.read(2) == 'G' )
  {
    ...
    config.SecondColor =ReadStringFromEEPROM(192);
    config.UnitMinuteColor =ReadStringFromEEPROM(202);
    config.TenthMinuteColor =ReadStringFromEEPROM(212);
    config.UnitHourColor =ReadStringFromEEPROM(222);
    config.TenthHourColor =ReadStringFromEEPROM(232);
    config.BlinkColor =ReadStringFromEEPROM(242);
    config.ShowSeconds = EEPROM.read(252);
    config.BlinkingEffect = EEPROM.read(254);
    return true;
  }
  else
  {
...
void printConfig(){
  ...
  Serial.printf("Second Color:%s\n", config.SecondColor.c_str());
  Serial.printf("Unit Minute Color:%s\n", config.UnitMinuteColor.c_str());
  Serial.printf("Tenth Minute Color:%s\n", config.TenthMinuteColor.c_str());
  Serial.printf("Unit Hour Color:%s\n", config.UnitHourColor.c_str());
  Serial.printf("Tenth Hour Color:%s\n", config.TenthHourColor.c_str());
  Serial.printf("Blink Color:%s\n", config.BlinkColor.c_str());
  Serial.printf("ShowSeconds:%d\n", config.ShowSeconds);
  Serial.printf("BlinkingEffect:%d\n", config.BlinkingEffect);
    // Application Settings here... from EEPROM 192 up to 511 (0 - 511)
